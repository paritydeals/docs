...
```shell
npm install paritydeals-sdk
```
or

```shell
yarn add paritydeals-sdk
```
...
```typescript
import { ParityDeals } from 'paritydeals-sdk';
...
```typescript
import { ParityDeals, CHARGE_PERIOD_CHOICES, FeatureListItem } from 'paritydeals-sdk';
...
* `chargePeriod` : Required. The billing cycle for the subscription. Must be one of:
...
**Optional Parameters:**
    ...
    * `paywallId` : Required. The id (uuid) of the paywall being used for this checkout. (Considered only if `offeringId` is not provided)

    * `ruleId` : Required. The id (uuid) of the pricing rule being applied. (Considered only if `offeringId` is not provided)

    * `customerId` : Required. The ID of the customer initiating the checkout. (If not provided, a new customer will be created)
...
```typescript
import { BEHAVIOUR_CHOICES } from 'paritydeals-sdk';
...
```typescript
import { CHARGE_PERIOD_CHOICES } from 'paritydeals-sdk';
...
* `subscriptionId` : Required. The unique identifier of the subscription to be updated.
* `planIdentifier` : Required. The identifier of the new plan.
* `chargePeriod` : Required. The new charging period for the subscription. Must be one of:
    * `"ONE_TIME"`
    * `"MONTHLY"`
    * `"YEARLY"`
    * `"WEEKLY"`
...
* `paywallId` :  The ID of the new paywall, if applicable.
...
* `message` : A confirmation message indicating the result of the update request.
...
```typescript
import { CANCELLATION_TYPES } from 'paritydeals-sdk';
