...
response = await async_client.checkout.create_session(
    ...
    paywallId="6db2717a-4e43-4ae1-a293-6c0a775f6264",
...
**Optional Parameters:**
    ...
    * `paywallId` (str): Required. The id (uuid) of the paywall being used for this checkout. (Considered only if `offeringId` is not provided)
...
* `paywallId` (str): Optional. The ID of the new paywall, if applicable.
