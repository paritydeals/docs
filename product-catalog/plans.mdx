...
<CardGroup cols={3}>
  ...
  <Card title="Enterprise/Custom">
    For plans that require custom contracts and manual invoicing.
  </Card>
</CardGroup>
...
ParityDeals provides several ways to configure features to support various business models.
...
The order of entitlement is critical as it determines their default display order in **pricing tables** that are rendered using ParityDeals. You can reorder entitlements to highlight your most important entitlements first. This ordering can be managed in two places:

1.  **From the Plan Details Page:** In the "Entitlements" section of a plan, you can change the order by using the drag-and-drop handle that appears on the left of each entitlement row. This sets the default order for this specific plan.

    <Frame>
    <img height="200" src="/images/reorder-features.png" />   
    </Frame>

2.  **From the Pricing Table Editor:** You can also override the default order for a specific pricing table directly within the Pricing Table editor, using a similar drag-and-drop interface.

    <Frame>
    <img height="200" src="/images/reorder-pricing-table.png" /> 
    </Frame>
...
While entitlements have a default name, you often need more customer-friendly or descriptive text for your pricing tables. ParityDeals allows you to override the default text for any entitlement on a specific pricing table.

1.  From the sidebar, navigate to **Monetization > Pricing Tables**.
2.  Select the pricing table you wish to edit from the list.
3.  In the pricing table editor, find the plan you want to modify and locate its feature list.
...
5.  A form will appear with the following fields:
    * **Customer display text:** This is the main text the customer will see for the feature in the pricing table (e.g., changing "api_calls_limit" to "Monthly API Requests").
...
6.  Click **"Save"** to apply the changes to your pricing table.
---
