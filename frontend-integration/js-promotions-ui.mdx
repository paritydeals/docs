---

title: "JS Promotion UI"
description: "Documentation for integrating the ParityDeals JavaScript SDK to display dynamic and localized pricing on your website."
icon: "code"
---

# Quick Start

Get dynamic, localized pricing on your site in just two steps:

## 1. Add the ParityDeals Script
Paste this snippet just before the closing `</head>` tag on your pricing page:

```html
<script type="text/javascript">
function resolvePdSDKFunction(e,...t){return new Promise(((n,i)=>{!function o(){window.ParityDealsSDK&&"function"==typeof window.ParityDealsSDK[e]?window.ParityDealsSDK[e](...t).then(n).catch(i):setTimeout(o,100)}()}))}!function(e,t,n,i,o,r,a){e[i]=e[i]||function(){(e[i].q=e[i].q||[]).push(Array.prototype.slice.call(arguments))},r=t.createElement(n),a=t.getElementsByTagName(n)[0],r.id="parity-deals-sdk",r.async=1,r.src="https://cdn.paritydeals.com/js-promotions-ui/js-promotions-ui-1.0.0.umd.js",r.type="module",a.parentNode.insertBefore(r,a)}(window,document,"script","ParityDealsSDK"),window.ParityDeals={init:function(e){ParityDealsSDK("init",e)},getUpdatedPrice:function(e,t){return resolvePdSDKFunction("getUpdatedPrice",e,t)},updatePriceElement:function(e,t){return resolvePdSDKFunction("updatePriceElement",e,t)},updatePrice:function(e){return resolvePdSDKFunction("updatePrice",e)}};
</script>
```

## 2. Initialize the SDK

Copy your product ID from the listing or promotion details page:

```js
ParityDeals.init({
  productId: 'YOUR_PRODUCT_ID',
  showBanner: true,
});
```
This will automatically display a discount banner if the user is eligible for a discount.

---

# Displaying Dynamic Prices (Basic)

The easiest way to show dynamic prices is to add the `data-pd-original-price` attribute to a wrapper element, and place a child element with `data-pd-price-formatted` where you want the price to appear. The SDK will automatically update this with the correct price for each visitor.

**Example:**

```html
<div class="pricing-plan" data-pd-original-price="99">
  <h3>Basic Plan</h3>
  <h3 data-pd-price-formatted>$99</h3>
</div>
```

**That's it!** The SDK will find and update these prices automatically.

---

# Customizing Price Display (Basic)

If you want to style different parts of the price (like the currency symbol or decimals), you can break the price into parts using these attributes **inside** a container with `data-pd-original-price`:

| Data Attribute             | Description                                      |
| :------------------------- | :----------------------------------------------- |
| `data-pd-price-formatted`  | The full formatted price (e.g., "$99.50")        |
| `data-pd-currency-symbol`  | Only the currency symbol (e.g., "$")             |
| `data-pd-price-integer`    | Integer part of the price (e.g., "99")           |
| `data-pd-price-decimal`    | Decimal part (e.g., "50")                        |
| `data-pd-price-decimal-point` | The decimal separator (e.g., ".")             |
| `data-pd-currency-code`    | 3-letter currency code (e.g., "USD")             |

**Example:**

```html
<div class="price-container" data-pd-original-price="99">
  <span class="currency" data-pd-currency-symbol></span>
  <span class="integer" data-pd-price-integer></span>
  <span class="decimal" data-pd-price-decimal></span>
</div>
```

---

# SDK Configuration Options

| Option               | Type    | Default  | Description                              |
| -------------------- | ------- | -------- | ---------------------------------------- |
| `productId`          | string  | null     | The product ID to display               |
| `showBanner`         | boolean | false    | Whether to show the discount banner      |
| `localizePricing`    | boolean | false    | Convert to local currency               |
| `baseCurrencyCode`   | string  | null     | The base currency code                  |
| `baseCurrencySymbol` | string  | null     | The base currency symbol                |
| `currencyDisplay`    | string  | null     | The currency display format             |
| `showDecimal`        | boolean | `true`   | Whether to show decimals                 |
| `minimumDecimalDigits` | number | 0        | The minimum number of decimal digits     |
| `maximumDecimalDigits` | number | 2        | The maximum number of decimal digits     |
| `isOriginalDisplay`    | boolean | false    | Whether to show the original price        |
| `bar.*`              | object  | —        | Customization for the discount banner    |
| `bar.unStyled`       | boolean | false    | Whether to show the discount banner    |
| `bar.showCloseButton`| boolean | true     | Whether to show the close button    |
| `bar.placement`      | string  | 'top'    | The placement of the discount banner    |
| `bar.container`      | string  | null     | The container for the discount banner    |
| `bar.backgroundColor`| string  | '#f0f0f0'    | The background color of the discount banner    |
| `bar.fontColor`      | string  | '#333333'    | The font color of the discount banner    |
| `bar.borderRadius`   | string  | '0px'    | The border radius of the discount banner    |
| `bar.fontSize`       | string  | '14px'   | The font size of the discount banner    |

---

# Advanced Usage

## Grouping and Strikethrough Prices

For more complex pricing layouts, you can use these advanced attributes:

* `data-pd-original-price-display`: Shows a "compare at" or strikethrough price (never discounted, but can be localized).
* `data-pd-rel`: Groups related price elements so they update together (e.g., main price and strikethrough).

**Example:**

```html
<div class="pricing-card">
  <h3>Pro Plan</h3>

  <div class="price-container" data-pd-original-price="99" data-pd-rel="pro-plan">
    <span class="currency" data-pd-currency-symbol></span>
    <span class="integer" data-pd-price-integer></span>
    <span class="decimal" data-pd-price-decimal></span>
  </div>

  <div class="original-price" data-pd-original-price-display="120" data-pd-rel="pro-plan">
    <s>
      <span data-pd-currency-symbol></span>
      <span data-pd-price-integer></span>
    </s>
  </div>
</div>
```

You can assign the same `data-pd-rel` value to all elements that should update together.

---

# API Reference (Advanced)

All SDK functions are asynchronous and return a `Promise`.

## `getUpdatedPrice`
Calculate and return the updated price for a given base price (does not update the DOM):

```js
const originalPrice = 99.99;
const options = { localizePricing: true };

ParityDeals.getUpdatedPrice(originalPrice, options)
  .then(result => {
    console.log('Updated price object:', result);
  });
```

## `updatePriceElement`
Update a specific DOM element's price:

```js
const priceElement = document.querySelector('#basic-price');
const options = { roundTo: 'up' };

ParityDeals.updatePriceElement(priceElement, options)
  .then(result => {
    console.log('Price element updated successfully.');
  });
```

## `updatePrice`
Bulk update multiple price elements with custom templates:

```js
const priceUpdates = [
  {
    element: document.querySelector('#price-1'),
    price: 10,
    options: { localizePricing: true },
    template: '<div class="price">{{formattedPrice}}</div>'
  },
  {
    element: document.querySelector('#price-2'),
    price: 20,
    options: { isOriginalDisplay: true },
    template: '<span>{{currencySymbol}}</span>{{integerPart}}'
  }
];

ParityDeals.updatePrice(priceUpdates)
  .then(results => {
    console.log('All prices updated:', results);
  });
```

---

# Support

Need help or have questions? Reach out to us anytime at **[hi@paritydeals.com](mailto:hi@paritydeals.com)**.