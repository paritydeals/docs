...
## Advanced Paywall Features & Customization

ParityDeals Paywalls offer advanced functionalities, primarily configured and controlled through our SDKs, giving developers flexibility in how the paywall behaves within their application.

### Showing Current Plan & Change Options
For logged-in users who already have a subscription, the paywall can intelligently display their current plan and dynamically present appropriate options to upgrade, downgrade, or change committed quantities. This requires passing customer context to the SDK.

```jsx
// Example for React SDK (Illustrative)
import { Paywall, SubscribeIntentionType } from '@paritydeals/react-sdk'; // Fictional SDK import name

function MySubscriptionManagementPage({ offeringId, customerContext }) {
  return (
    <Paywall
      offeringId={offeringId} // Your specific Offering ID
      customer={customerContext} // Pass customer's current subscription details
      onPlanSelected={({ plan, customer, intentionType }) => {
        console.log("Plan selected:", plan.id, "Intention:", intentionType);
        // Handle different user intentions based on the 'intentionType'
        switch (intentionType) {
          case SubscribeIntentionType.START_TRIAL:
            // TODO: Implement logic to provision a trial subscription for the selected plan
            break;
          case SubscribeIntentionType.REQUEST_CUSTOM_PLAN_ACCESS:
            // TODO: Implement logic to redirect to a contact form or sales process
            break;
          case SubscribeIntentionType.CHANGE_UNIT_QUANTITY: // For plans with customizable quantities
          case SubscribeIntentionType.UPGRADE_PLAN:
          case SubscribeIntentionType.DOWNGRADE_PLAN:
            // TODO: Implement logic to launch your checkout flow with the selected plan
            break;
          case SubscribeIntentionType.CANCEL_SCHEDULED_UPDATES: // If user cancels a pending change
            // TODO: Implement logic to confirm cancellation and remove scheduled updates from their subscription
            break;
        }
      }}
    />
