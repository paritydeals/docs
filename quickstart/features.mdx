...
*[Image: A composite diagram showing a "Feature" in the main features list, and an arrow pointing to it being used as an "Entitlement" with a specific rule inside a Plan's configuration.]*
...
2.  Click the **"+ Create feature"** button in the top-right corner.
    ...
    *[Image: The main "Features" list page with the "+ Create feature" button highlighted.]*
...
3.  In the "Create feature" modal, fill in the following primary details:
    ...
    *[Image: The main "Create feature" modal showing the Name, Identifier, and Description fields being filled out.]*
...
*[Image: The Feature Type dropdown with "Switch/boolean" selected.]*
...
*[Image: The feature unit editor showing input fields for singular ("Project") and plural ("Projects") forms.]*
...
*[Image: The creation modal with "Metered" and "Pre-aggregated usage" selected, showing the "Use different feature units for customers" toggle.]*
...
4.  **Define Event Filters:** Specify which events should count towards usage. You can filter by `event name` and add other properties from the event payload.
    ...
    *[Image: The Event Filters UI showing conditions with AND/OR logic and the live event preview on the right.]*
...
5.  **Configure Aggregation:** Specify how ParityDeals should calculate a single value from the filtered events. All methods besides `Count` require you to select an event property to aggregate.
    ...
    *[Image: The Aggregation UI showing the different aggregation methods (Count, Sum, etc.) and the "Over property" dropdown.]*

6.  Click **"Create"** to save the feature and its aggregation rules. You will see a success screen.

*[Image: The success screen after creating a raw usage feature.]*
...
*[Image: The unit conversion configuration section in the feature creation modal, showing base units, customer-facing units, and the conversion rate.]*
...
*[Image: The detail page of a feature in edit mode, with the "Save updates" button highlighted.]*
...
*[Image: The three-dot menu on a feature row with the "Archive" option highlighted.]*