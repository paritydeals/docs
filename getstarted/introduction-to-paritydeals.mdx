...
- **Meter usage in real time**  
 Count every API hit, data storage unit, or AI tokens in real time. Set soft or hard caps, pick reset cycles, send alerts, and handle overages with one call.

- **Control feature access**  
  Define features per plan. Turn them on or off and check access in your app with one SDK call.

- **Localized pricing by country**  
  Offer country-specific prices in 135+ currencies with VPN, proxy, and fraud protection built in. You can also set up promotions for specific countries.
...
- Works with **any payment processor** — Stripe, Gumroad, LemonSqueezy, Dodo Payments, Polar, and more  
