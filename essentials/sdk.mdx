...
## **Table of Contents**
 ...
 1. [Overview](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#overview)
 2. [Installation](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#installation)
 3. [Setup & Configuration](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#setup--configuration)
    - [ParityDealsProvider](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#paritydealsprovider)
    - [Required Props](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#required-props)
    - [Optional Props](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#optional-props)
    - [Configuration Object (](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#configuration-object-config)`config`[)](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#configuration-object-config)
 4. [Fetching Entitlements](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#fetching-entitlements)
    - [Automatic Fetching on Mount](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#automatic-fetching-on-mount)
    - [Manual Fetching](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#manual-fetching)
 5. [Accessing Entitlement Data (Hooks)](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#accessing-entitlement-data-hooks)
    - [useParityDeals](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#useparitydeals)
    - [useAllEntitlements](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#useallentitlements)
    - [useBooleanEntitlement](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#usebooleanentitlement)
    - [useConfigEntitlement](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#useconfigentitlement)
    - [useMeteredEntitlement](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#usemeteredentitlement)
 6. [Conditional Rendering Components](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#conditional-rendering-components)
    - [ShowWhenBooleanEntitled](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#showwhenbooleanentitled)
    - [ShowWhenConfigEntitled](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#showwhenconfigentitled)
    - [ShowWhenMeteredEntitled](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#showwhenmeteredentitled)
 7. [Key Types](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#key-types)
 8. [Error Handling](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#error-handling)
 9. [Example Usage](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#example-usage)
10. [API Request Service](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#api-request-service)
11. [Project Structure (for contributors/reference)](https://gemini.google.com/u/2/app/aa1fd84a851e1cf4#project-structure-for-contributorsreference)
...
import { ParityDealsProvider, ParityDealsApiBehaviorOptions } from '@paritydeals/react-sdk';
// Assuming your apiRequest service is correctly set up and imported if needed by the SDK internally
// For this SDK, it's assumed to be bundled or correctly resolved.

const App = () => {
  const staticAuthToken = "YOUR_STATIC_API_TOKEN"; // Replace with your actual token

  const providerBehaviorOptions: ParityDealsApiBehaviorOptions = {
    onError: (error) => {
      console.error("ParityDeals SDK Global Error:", error.message);
      // Add your error reporting logic here (e.g., Sentry, LogRocket)
    },
    fetchEntitlementsOnMount: true, // Default is true
    // maxRetries: 3, // Optional
    // timeout: 5000, // Optional
    // offeringId: "your-offering-id" // Optional, if your API uses this as a query param
  };

  return (
    <ParityDealsProvider
      apiUrl="https://edge.your-api.com/api/v1" // Base URL of your ParityDeals API
      entitlementsPath="entitlement-check"     // Path to the entitlements endpoint
      orgId="your-organization-id"
      customerId="your-customer-id"
      authToken={staticAuthToken}
      config={providerBehaviorOptions}
    >
      <MyAppContent />
    </ParityDealsProvider>
  );
...
const MyAppContent = () => {
  // Your application components that can now use ParityDeals hooks
...
- `authToken?: string | null`
  - Your static API token or Bearer token for authenticating requests to the ParityDeals API. If provided, it will be included in the `Authorization: Bearer <authToken>` header.
...
1. Wrapping your application with `ParityDealsProvider` and providing the required props (`apiUrl`, `entitlementsPath`, `orgId`, `customerId`) and any optional `config` or `authToken`.
...
The SDK relies on an `apiRequest` function for making HTTP requests. The SDK's `ParityDealsProvider` passes configuration like `timeout`, `maxRetries`, `backoffBaseDelay`, and `authToken` to this function.

- **Your Responsibility:** You need to ensure that your project includes an implementation of this `apiRequest` service (e.g., in `src/services/makeRequest.service.ts`) that matches the `ApiRequestConfig` and `ApiResponse` interfaces used by the SDK. This service should handle actual HTTP communication, including appending query parameters passed via `ApiRequestConfig.queryParams` and using the `authToken` to set the `Authorization` header.
